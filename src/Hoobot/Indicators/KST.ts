...
export const calculateROC = (candles: Candlestick[], period: number, source: string = 'close'): Candlestick[] => {
  ...
  }).slice(period).map(roc => ({ close: roc } as Candlestick)); 
};
...
export const calculateKST = (
  ...
  const signalLine = calculateSMA([{ close: kst[0] } as Candlestick, ...(kst.map(k => ({ close: k })) as Candlestick[])], SigLen, "close").slice(1); 
